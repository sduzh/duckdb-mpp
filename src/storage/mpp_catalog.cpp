...
unique_ptr<PhysicalOperator> MppCatalog::PlanInsert(ClientContext &context, LogicalInsert &op,
	...
	auto insert = make_uniq<PhysicalShuffleWrite>(
	    op.types, op.estimated_cardinality, *shuffle_manager_, op.table, op.column_index_map,
	    std::move(op.bound_defaults), std::move(hash_expression), std::move(shard_locations), local_endpoint_);
	if (plan) {
...
unique_ptr<PhysicalOperator> MppCatalog::PlanUpdate(ClientContext &context, LogicalUpdate &op,
                                                    unique_ptr<PhysicalOperator> plan) {
	throw NotImplementedException("MppCatalog:PlanUpdate");
}
